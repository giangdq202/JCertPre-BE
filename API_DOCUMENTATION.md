...
| Method | Endpoint | Description | Parameters |
|--------|----------|-------------|------------|
| `DELETE` | `/clear-all` | Clears all cache entries | None |
...
| `GET` | `/` | Gets livestreams with pagination and filters | Query: `courseId`, `searchTerm`, `pageIndex`, `pageSize` |
| `GET` | `/course/{courseId}` | Gets livestreams by course ID | Path: `courseId` |
| `GET` | `/{id}/join-token` | Generates join token for livestream | Path: `id`, Query: `userId` |
| `POST` | `/{id}/start` | Starts a livestream | Path: `id`, Query: `userId` |
| `GET` | `/{id}/can-join` | Checks if user can join livestream | Path: `id`, Query: `userId` |
| `GET` | `/user/{userId}` | Gets livestreams by user ID | Path: `userId` |
| `GET` | `/user/{userId}/timetable` | Gets livestream timetable by user | Path: `userId` |
...
| `GET` | `/user-enrollments` | Gets all enrollments for user | Query: `userId` |
| `GET` | `/enrollment-status` | Checks enrollment status | Query: `userId`, `courseId` |
...
| `GET` | `/{id}` | Gets study plan by ID | Path: `id` (Guid) |
| `POST` | `/create` | Creates a new study plan | Body: `CreateStudyPlanDto` |
| `PUT` | `/{id}` | Updates study plan | Path: `id`, Body: `UpdateStudyPlanDto` |
| `DELETE` | `/{id}` | Deletes study plan | Path: `id` (Guid) |

### Study Plan Items APIs
**Base Route:** `/api/study-plan-items`

| Method | Endpoint | Description | Parameters |
|--------|----------|-------------|------------|
| `GET` | `/by-study-plan/{studyPlanId}` | Gets items by study plan ID | Path: `studyPlanId` |
| `POST` | `/` | Creates study plan item | Body: `CreateStudyPlanItemDto` |
| `PUT` | `/{id}` | Updates study plan item | Path: `id`, Body: `UpdateStudyPlanItemDto` |
| `DELETE` | `/{id}` | Deletes study plan item | Path: `id` (Guid) |
